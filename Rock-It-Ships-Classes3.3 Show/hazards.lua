...
local blackhole = display.newCircle(display.contentCenterX, display.contentCenterY, 300)
blackhole.alpha = 0.3
physics.addBody( blackhole, "static", { isSensor=true, radius = 300} )
...
local function onCollision( event )
	...
	if ( event.phase ~= "ended" ) then
		--blackhole
		if(obj1.type == "blackhole") then
			print("blackhole")
			objAngle = math.rad(math.atan((obj2.y-display.contentCenterY)/(obj2.x-display.contentCenterX)))
			objXComp = math.cos(objAngle)
			objYComp = math.sin(objAngle)
			obj2:applyLinearImpulse(10*forceMag*objXComp, 10*forceMag*objYComp, obj2.x, obj2.y)
		end
	end
