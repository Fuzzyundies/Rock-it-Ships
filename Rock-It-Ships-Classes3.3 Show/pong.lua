...
local objectSheet5 = graphics.newImageSheet("pongsteroid.png", sheetOptions5)
...
local player1ScoreText
local player2ScoreText
local winnerText
local counter = 0
local scored = false
...
local playerCollisionFilter = { categoryBits = 1, maskBits = 15}
local laserCollisionFilter = { categoryBits = 2, maskBits = 5 }
local asteroidCollisionFilter = { categoryBits = 4, maskBits = 11}
local bumperCollisionFilter = { categoryBits = 8, maskBits = 5 }
...
function ship.new(name)
  ...
  physics.addBody(self.body,"dynamic", {radius = 25, filter = playerCollisionFilter})
...
function ship:fire()
	...
	physics.addBody( self.laser, "dynamic", {radius = 25, filter = laserCollisionFilter} )
...
function explosion(xCoord, yCoord)
	...
	transition.to( exp, {time = 250, alpha = 1, xScale = 0.7, yScale = 0.7} )
	transition.to( exp, {delay = 250, time = 250, alpha = 0, onComplete = function()display.remove(exp)end})
end

local function updateText()
	--explode = true
	player1ScoreText = "Player 1 Score: ".. player1Score
	player2ScoreText = "Player 2 Score: ".. player2Score
end
...
local function createAsteroid()
	...
	physics.addBody(newAsteroid,"dynamic", { radius = 80, bounce = 1.0, filter = asteroidCollisionFilter})
	newAsteroid.name = "asteroid"
	newAsteroid.type = "asteroid"
	--newAsteroid.health = 3
	newAsteroid.hit = false
	newAsteroid.x = display.contentWidth/2
		newAsteroid.y = display.contentHeight/2
		newAsteroid:setLinearVelocity(math.random(-120,120),math.random(-120,120))
...
local function makeAsteroidBumpers()
	local fudgeNum = 75 + 2 -- extra positioning when rock is moved
	
	asteroidBumper["top"] = display.newRect( display.contentWidth/2,0 - fudgeNum, display.contentWidth + fudgeNum *2, fudgeNum )	
	...
	physics.addBody( asteroidBumper["top"], "dynamic", {density = 10, friction = 10,bumperCollisionFilter})

	asteroidBumper["left"] = display.newRect( 0 - fudgeNum, display.contentHeight/2, fudgeNum, display.contentHeight + fudgeNum*2 )
	...
	physics.addBody( asteroidBumper["left"], "dynamic", {density = 10, friction = 10,bumperCollisionFilter})

	asteroidBumper["bottom"] = display.newRect( display.contentWidth/2, display.contentHeight + fudgeNum, display.contentWidth + fudgeNum * 2, fudgeNum )
	...
	physics.addBody( asteroidBumper["bottom"], "dynamic", {density = 10, friction = 10,bumperCollisionFilter})

	asteroidBumper["right"] = display.newRect( display.contentWidth + fudgeNum, display.contentHeight/2, fudgeNum, display.contentHeight+fudgeNum*2 )
	...
	physics.addBody( asteroidBumper["right"], "dynamic", {density = 10, friction = 10,bumperCollisionFilter})
end
...
local function onKeyEvent(event)
	if(player1Score < 3 and player2Score < 0) then
		local thisPlayer
...
local function onAxisEvent(event)
	if(player1Score < 3 and player2Score < 3) then
...
local function restoreAsteroid()
	if(player1Score < 3 and player2Score < 3) then
		createAsteroid()
	end
end
...
local function onCollision( event )
	...
	if ( event.phase == "began" ) then
		...
		--Asteroid Bumper
		if (obj1.type == "asteroidBumper" and (obj1.name == "left" or obj1.name == "right") and obj2.type == "asteroid" and scored == false) then
        	...
        	local function score()
        		...
        		if(player1Score == 3 or player2Score == 3) then
					...
					if (player1Score == 3) then
						winnerText = display.newText( uiGroup, "PLAYER 1 WINS", display.contentWidth/2, 150, inputText, 30 )
					else
						winnerText = display.newText( uiGroup, "PLAYER 2 WINS", display.contentWidth/2, 150, inputText, 30 )
					end
					timer.performWithDelay( 4000, endGame )
				else
	        		--print("not getting here")
	        		timer.performWithDelay(3000, restoreAsteroid)
	        	end
        	end
        	timer.performWithDelay (1, score )


        elseif (obj1.type == "asteroidBumper" and obj2.type == "laser") then
		...
		elseif ((obj1.type == "laser" and obj2.type == "smallAsteroid" and obj2.hit == false) or 
				(obj1.type == "smallAsteroid" and obj1.hit == false and obj2.type == "laser")) then
			if (obj1.type ~= "smallAsteroid") then
				obj3 = obj1
				obj1 = obj2
				obj2 = obj3
				obj3 = nil
		...
		elseif ((obj1.type == "ship" and obj1.hit == false) and (obj2.type == "ship" and obj2.hit == false)) then
			...
			timer.performWithDelay(100,collision)
			...
			local function hit()
				...
				if(player2.shieldIsActive == false) then
					...
					if (obj1.health <= 0) then
						...
						explosion(player2.body.x, player2.body.y)
...
function scene:create( event )
	...
	player1ScoreText = display.newText(uiGroup, "Player 1 Score: ".. player1Score, 200, 20, inputText, 20)
	player2ScoreText = display.newText(uiGroup, "Player 2 Score: ".. player2Score, display.contentWidth - 200, 20, inputText, 20)
