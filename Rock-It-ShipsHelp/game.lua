...
local function createAsteroid()
		...
		newAsteroid:scale(0.9,0.9)
		table.insert(asteroidsTable, newAsteroid)
		physics.addBody(newAsteroid,"dynamic", { radius = 35, bounce = 0.5})
...
local function makeAsteroidBumpers()
	...
	asteroidBumper["top"] = display.newRect(hiddenGroup, screenLeft, screenTop-fudgeNum, screenWidth + fudgeNum, 2 )	
	...
	asteroidBumper["left"] = display.newRect(hiddenGroup, screenLeft-fudgeNum, screenTop, 2, screenHeight+fudgeNum+2 )
	...
	asteroidBumper["bottom"] = display.newRect(hiddenGroup, screenLeft-5, screenBottom+fudgeNum, screenWidth+10 + fudgeNum, 2 )
	...
	asteroidBumper["right"] = display.newRect(hiddenGroup, screenRight+fudgeNum, screenTop, 2, screenHeight+fudgeNum+2 )
...
local function makeOtherBumpers()
	local fudgeNum = 37 + 2 -- extra positioning when rock is moved
	
	otherBumper["top"] = display.newRect(hiddenGroup, screenLeft, screenTop-fudgeNum, screenWidth + fudgeNum, 2 )	
	...
	otherBumper["left"] = display.newRect(hiddenGroup, screenLeft-fudgeNum, screenTop, 2, screenHeight+fudgeNum+2 )
	otherBumper["left"].type = "asteroidBumper"
	...
	otherBumper["bottom"] = display.newRect(hiddenGroup, screenLeft-5, screenBottom+fudgeNum, screenWidth+10 + fudgeNum, 2 )
	otherBumper["bottom"].type = "asteroidBumper"
	...
	otherBumper["right"] = display.newRect(hiddenGroup, screenRight+fudgeNum, screenTop, 2, screenHeight+fudgeNum+2 )
	otherBumper["right"].type = "asteroidBumper"
...
local obj1 = event.object1
	...
	if ( event.phase == "began" ) then
		...
		elseif (obj1.type == "otherBumper" and obj2.type ~= "asteroid") then
        	local function wrap()
        		if obj1.name == "left" or obj1.name == "right" then
		...
		elseif ( ( obj1.myName == "laser1" or obj1.myName == "laser2") and obj2.myName == "asteroid")
		then
			-- Remove both the laser and asteroid
			display.remove( obj1 )
			for i = #asteroidsTable, 1, -1 do
				if ( asteroidsTable[i] == obj2 ) then
					asteroidsTable[i].health = asteroidsTable[i].health - 1
					if (asteroidsTable[i].health <= 0) then
						table.remove( asteroidsTable, i )
						asteroidCount = asteroidCount -1
						display.remove( obj2 )
					end
...
function scene:create( event )
	...
	physics.addBody(ship1, {radius = 15, isSensor = true})
	ship1.myName = "ship1"
	ship1:rotate(90)
	...
	physics.addBody(ship2, {radius = 15, isSensor = true})
	ship2.myName = "ship2"
	ship2:rotate(-90)
